...
function getGroqClient() {
  ...
  return new Groq({
    apiKey: apiKeys[currentApiKeyIndex % apiKeys.length],
  })
}

export function switchGroqApiKey() {
  currentApiKeyIndex++
}
...
Return only the question. Nothing else.`
    ...
    try {
      ...
      const question = message.choices[0]?.message?.content?.trim() || ""
      if (question && question.length > 20 && !question.includes("**")) return question
    } catch (err: any) {
      if (err?.status === 429) {
        console.warn("HR intro: Groq quota exceeded, switching API key")
        switchGroqApiKey()
...
Question:`
  ...
  try {
    ...
    let question = message.choices[0]?.message?.content?.trim() || ""
    ...
    if (err?.status === 429) {
      console.warn("Question generation: Groq quota exceeded, switching API key")
      switchGroqApiKey()
      throw new QuotaExceededError(60)
    }
  ...
  const fallbackList = role === "hr" ? fallbackQuestions.hr : role === "technical" ? fallbackQuestions.technical : fallbackQuestions.manager
...
Highlights:
- Experience: ${experiences.map((e) => `${e.position || "Role"} at ${e.company || "Company"}`).join(" → ") || "Not provided"}
- Leadership Skills: Team management, project delivery, stakeholder communication
- Summary: ${summary}`
}
...
Respond with only valid JSON, no additional text.`
  ...
  try {
    ...
    if (error?.status === 429) {
      console.warn("Evaluation: Groq quota exceeded, switching API key")
      switchGroqApiKey()
      throw new QuotaExceededError(60)
    }
...
export async function transcribeAudio(audioBuffer: Buffer): Promise<string> {
  ...
  // In production, integrate with Groq transcription or Google Cloud Speech-to-Text
